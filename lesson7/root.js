...
function WelcomeMessage({ name, country }) {
    return <li>Welcome {name} from {country}</li>;
}
...
function WelcomeMessagesList() {
    return (
        <ul>
            {users.map(user => (
                <>
                    {user.country === 'Singapore' && <li key={`majulah-${user.name}`}>Majulah Singapura, {user.name}!</li>}
